...
class AvgPool(Module):
    ...
    def _simple_lrp(self,R):
        ...
        result = self.compute_result(image_patches)
    ...
    def _epsilon_lrp(self,R, epsilon):
        ...
        self.check_shape(R)
        image_patches = self.extract_patches()
        result = self.compute_result(image_patches, epsilon)
        return self.restitch_image(result)
    ...
    def compute_zs(self, Z, stabilizer=True, epsilon=1e-12):
        Zs = tf.reduce_sum(Z, [3,4], keep_dims=True)  
        if stabilizer==True:
    ...
    def compute_result(self, Z, epsilon=1e-12):
        ...
        # result = Z * tf.reshape((self.R/tf.reshape(Zs, tf.shape(self.R))), [self.in_N,self.Hout,self.Wout,1,1,self.in_depth])
